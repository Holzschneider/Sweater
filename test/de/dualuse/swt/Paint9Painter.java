...
class Paint9Painter {
    ...
    public static boolean validImage(Image image) {
        ...
        return (image != null && b.width > 0 && b.height > 0);
    }
     ...
     */
    public void paint(Control c, GC g, int x, int y, int w,
            int h, Image source, Insets sInsets, Insets dInsets,
            PaintType type, int mask) {
        if (source == null) {
            return;
        }
        
        paint9( g, x, y, w, h, source, sInsets, dInsets, type,
                mask);
    }

//    protected void paintToImage(Component c, Image destImage,
//            Graphics g, int w, int h, Object[] args) {
//        int argIndex = 0;
...
//    }
    ...
    protected void paint9(GC g, int x, int y, int w, int h,
            Image image, Insets sInsets, Insets dInsets,
        ...
        if (type == PaintType.CENTER) {
            // Center the image
            g.drawImage(image, x + (w - iw) / 2, y + (h - ih) / 2);
            ...
            for (int yCounter = y, maxY = y + h; yCounter < maxY; yCounter += (ih - lastIY), lastIY = 0) {
                ...
                for (int xCounter = x, maxX = x + w; xCounter < maxX; xCounter += (iw - lastIX), lastIX = 0) {
                    ...
                    g.drawImage(image, 
                    		lastIX, lastIY, lastIX + dx2 - xCounter-lastIX, lastIY + dy2 - yCounter-lastIY,
                    		xCounter, yCounter, dx2-xCounter, dy2-yCounter
                            );
                }
            }
        } else {
            ...
            if ((componentMask & PAINT_LEFT) != 0) {
                drawChunk(image, g, stretch, x, y + dt, x + dl, y + h
                        - db, 0, st, sl, ih - sb, false);
            }
            if ((componentMask & PAINT_TOP_LEFT) != 0) {
                drawImage(image, g, x, y, x + dl, y + dt, 0, 0, sl, st);
            }
            if ((componentMask & PAINT_TOP) != 0) {
                drawChunk(image, g, stretch, x + dl, y, x + w - dr, y
                        + dt, sl, 0, iw - sr, st, true);
            }
            if ((componentMask & PAINT_TOP_RIGHT) != 0) {
                drawImage(image, g, x + w - dr, y, x + w, y + dt, iw
                        - sr, 0, iw, st);
            }
            if ((componentMask & PAINT_RIGHT) != 0) {
                drawChunk(image, g, stretch, x + w - dr, y + dt, x + w,
                        y + h - db, iw - sr, st, iw, ih - sb, false);
            }
            if ((componentMask & PAINT_BOTTOM_RIGHT) != 0) {
                drawImage(image, g, x + w - dr, y + h - db, x + w, y
                        + h, iw - sr, ih - sb, iw, ih);
            }
            if ((componentMask & PAINT_BOTTOM) != 0) {
                drawChunk(image, g, stretch, x + dl, y + h - db, x + w
                        - dr, y + h, sl, ih - sb, iw - sr, ih, true);
            }
            if ((componentMask & PAINT_BOTTOM_LEFT) != 0) {
                drawImage(image, g, x, y + h - db, x + dl, y + h, 0, ih
                        - sb, sl, ih);
            }
            if ((componentMask & PAINT_CENTER) != 0) {
                drawImage(image, g, x + dl, y + dt, x + w - dr, y + h
                        - db, sl, st, iw - sr, ih - sb);
            }
        }
    }

    private void drawImage(Image image, GC g, int dx1, int dy1,
     ...
     */
    private void drawChunk(Image image, GC g, boolean stretch,
            int dx1, int dy1, int dx2, int dy2, int sx1, int sy1,
        ...
        if (stretch) {
            ...
            while (dx1 < dx2 && dy1 < dy2) {
                ...
                g.drawImage(image, 
                		sx1, sy1, sx1 + newDX2 - dx1-sx1, sy1 + newDY2 - dy1-sy1,
                		dx1, dy1, newDX2-dx1, newDY2-dy1);
