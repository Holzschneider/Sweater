	...
	public double[] add(Contour b) {
		...
		for (int j=0, J=b.n; j<J; j++) {
			...
			for (int i=0, I=n; i<I; i++) {
				...
				double[] p = Contour.intersect(x1, y1, x2, y2, x3, y3, x4, y4, tmp);
				if (p==null) {
				...
				if (Double.isInfinite(p[0]) || Double.isNaN(p[0])) {					
					System.out.println("Debug me");
					p = Contour.intersect(x1, y1, x2, y2, x3, y3, x4, y4, tmp);
				}
...
//	static Path2D path;
	...
	static ArrayList<Contour> shapes = new ArrayList<>();
	static Contour path;
